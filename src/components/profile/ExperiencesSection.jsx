      ...
      <ul className="space-y-4">
        {experiences.map((exp, index) => (
          <li key={index} className="flex justify-between items-center">
            <div>
              <strong className="text-lg">{exp.title}</strong> - <span className="text-[#8D8A8A] font-semibold text-lg">{exp.company}</span>
            </div>
            <span className="text-base text-[#8D8A8A]">{exp.duration}</span>
          </li>
        ))}
      </ul>
    </div>
